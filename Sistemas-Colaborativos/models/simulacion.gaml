/**
* Name: Simulacion Av Simon Bolivar
* Author: Jordi, Byron
* Description: Lógica exacta
*/
...
// --- 1. ARCHIVOS ---
	...
	// --- 3. DATOS DE RIESGO ---
	list<list<float>>
	matriz_riesgo_semanal <- [[1.0, 0.5, 0.5, 0.5, 0.5, 1.5, 2.0, 2.0, 2.0, 1.0, 1.5, 1.5, 1.0, 2.0, 1.0, 1.0, 2.0, 2.5, 2.0, 1.0, 1.0, 1.0, 0.5, 1.0], [0.5, 0.5, 0.5, 0.5, 0.5, 1.0, 2.5, 2.5, 3.0, 2.0, 1.0, 1.0, 1.0, 1.0, 2.0, 2.5, 2.0, 1.5, 2.5, 1.0, 0.5, 0.5, 0.5, 1.5], [1.0, 1.0, 0.5, 0.5, 0.5, 1.5, 2.0, 1.0, 2.5, 1.0, 1.0, 1.0, 1.0, 2.0, 2.5, 1.0, 1.0, 2.0, 1.5, 2.0, 1.0, 0.5, 0.5, 1.0], [1.0, 1.0, 1.5, 0.5, 0.5, 2.5, 2.0, 2.5, 2.0, 1.0, 0.5, 1.5, 1.0, 0.5, 1.0, 1.0, 2.0, 2.5, 2.0, 1.5, 0.5, 0.5, 0.5, 2.0], [2.0, 1.5, 1.5, 1.0, 0.5, 1.5, 1.0, 2.5, 1.5, 1.0, 1.0, 1.0, 3.0, 1.0, 0.5, 2.0, 2.5, 3.0, 1.0, 1.5, 2.5, 2.0, 3.0, 2.5], [2.0, 2.0, 2.0, 1.0, 3.0, 3.0, 2.5, 1.5, 3.0, 1.5, 1.5, 2.5, 3.0, 1.0, 0.5, 2.5, 1.0, 0.5, 1.0, 2.0, 1.5, 2.0, 0.5, 1.0], [2.0, 1.5, 5.0, 1.0, 1.5, 2.5, 4.0, 2.5, 1.5, 1.5, 1.0, 1.5, 2.0, 2.0, 2.5, 2.5, 1.0, 2.5, 1.5, 1.5, 4.0, 1.0, 2.0, 0.5]];
	...
	int radio_deteccion <- 600; // Radio para detectar accidentes cercanos
	int num_celdas_calor <- 150; // Número de puntos de muestreo
	init {
	...
	// Acción para crear el grid del mapa de calor
	action crear_mapa_calor {
	// Crear celdas SOLO sobre la vía principal
	...
	reflex control_tiempo_y_clima {
		...
		if (minuto_actual >= 60) {
			...
			if (flip(prob_lluvia_mes at (mes_simulacion - 1))) {
				lluvia_activa <- true;
			} else {
				lluvia_activa <- false;
			}
		...
		if (hora_actual >= 19 or hora_actual <= 5) {
			...
			es_noche <- false;
			color_fondo <- lluvia_activa ? #dimgray : rgb(60, 60, 60);
		}
...
// Especie para el mapa de calor
species celda_calor {
	float intensidad <- 0.0;
	rgb color_calor;
	float tamano_circulo <- 0.0; // Tamaño dinámico del círculo

	// Calcular intensidad y tamaño basado en accidentes cercanos
	reflex actualizar_intensidad {
		intensidad <- 0.0;
		int accidentes_cercanos <- 0;

		// Contar y ponderar accidentes cercanos
		loop accidente over: ubicaciones_accidentes {
			float distancia <- location distance_to accidente;
			if (distancia < radio_deteccion) {
				accidentes_cercanos <- accidentes_cercanos + 1;
				// Mientras más cerca, más peso
				intensidad <- intensidad + (1.0 - (distancia / radio_deteccion));
			}

		}

		// Determinar COLOR y TAMAÑO según concentración
		if (intensidad = 0.0) {
			color_calor <- rgb(0, 0, 0, 0); // Transparente
			tamano_circulo <- 0.0;
		} else if (intensidad < 0.5) {
		// AMARILLO SUAVE - 1 accidente lejano
			color_calor <- rgb(255, 255, 100, 120);
			tamano_circulo <- 80.0;
		} else if (intensidad < 1.2) {
		// AMARILLO INTENSO - 1-2 accidentes
			color_calor <- rgb(255, 220, 0, 160);
			tamano_circulo <- 120.0;
		} else if (intensidad < 2.5) {
		// NARANJA - 3-4 accidentes cercanos
			color_calor <- rgb(255, 150, 0, 180);
			tamano_circulo <- 180.0;
		} else if (intensidad < 4.0) {
		// NARANJA-ROJO - 5-6 accidentes
			color_calor <- rgb(255, 80, 0, 200);
			tamano_circulo <- 250.0;
		} else {
		// ROJO INTENSO - Zona MUY CRÍTICA (7+ accidentes)
...
species Vehiculo skills: [moving] {
	...
	reflex moverse {
		...
		if (lluvia_activa) {
			velocidad_real <- velocidad_real * 0.7;
		}
	...
	action registrar_choque {
		...
		// Registrar ubicación del accidente para el mapa de calor
		ubicaciones_accidentes <- ubicaciones_accidentes + location;
		if (lluvia_activa and !es_ebrio) {
			acc_clima <- acc_clima + 1;
		} else if (es_ebrio) {
			acc_alcohol <- acc_alcohol + 1;
		} else if (es_imprudente) {
			acc_velocidad <- acc_velocidad + 1;
		} else if (no_respeta_distancia) {
...
experiment Simulacion_Calibrada type: gui {
	...
	output {
		...
		display mapa type: opengl background: color_fondo {
			...
			species via_decorativa;
			species via_principal;
			species celda_calor; // Mapa de Calor
			species Auto;
			species Moto;
			species Camioneta;
			species Bus;
